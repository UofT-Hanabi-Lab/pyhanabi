...
class AdaptedLLMAgent(Player):
    ...
    def _get_current_stack(self, board):
        ...
        for color, num in board:
            description += f"{self.color_name_map[color]} - {self.color_name_map[color]} {num} "
        description += '\n'
    ...
    def _format_actions(self, valid_actions):
        ...
        for i, action in enumerate(valid_actions):
            ...
            if action.action_type == Action.ActionType.PLAY:
                ...
                formatted.append(f"{ord_letter}. Hint Player {action.pnr}'s {self.color_name_map[action.col]} color cards.")
            elif action.action_type == Action.ActionType.HINT_NUMBER:
                formatted.append(f"{ord_letter}. Hint Player {action.pnr}'s rank {action.num} cards.")
        return formatted
    ...
    def _infer_partner_action(self, description):
        ...
        for player_idx in range(len(self.player_names)):
            if player_idx != self.player_id and self.last_actions[player_idx] is not None:
    ...
    def _observation_to_description(self, nr, hands, knowledge, trash, board, hints, deck_size):
        ...
        for player_idx in range(len(hands)):
            if player_idx != nr:
                self.working_memory[f'player_{player_idx}_hand'] = self._get_player_hand(hands, player_idx)
                description += self.working_memory[f'player_{player_idx}_hand']
                
                self.working_memory[f'player_{player_idx}_knowledge'] = self._get_card_knowledge(knowledge, player_idx)
        ...
        trash_str = ", ".join(f"{self.color_name_map[color]} {num}" for color, num in trash)
    ...
    def find_best_match(self, action_string):
        ...
        if match:
            ...
            if 'action:' in selected_match.lower():
                ...
                if match:
                    selected_match = match.group(1).strip().lower()
            selected_move, score = process.extractOne(selected_match, [t.lower() for t in self.transformed])
    ...
    def get_action(self, nr, hands, knowledge, trash, played, board, valid_actions, hints):
        ...
        self.working_memory['partner_interpretation'] = self._infer_partner_action(temp_description)
        ...
        if self.use_verification:
            ...
            counter = 0
            while 'verification: okay' not in verification_response_string.lower() and counter < 3:
                counter += 1
        ...
        max_len = max(len(v) for v in self.log_csv_dict.values()) if self.log_csv_dict else 0
        padded_dict = {k: v + [''] * (max_len - len(v)) for k, v in self.log_csv_dict.items()}
    ...
    def _action_to_string(self, action, player):
        ...
        if action.action_type == Action.ActionType.PLAY:
            ...
            return f"Player {player} hinted Player {action.pnr}'s rank {action.num} cards."
