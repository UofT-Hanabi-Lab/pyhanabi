...
class IntentionalPlayer(Player):
    ...
    def get_action(self, nr, hands, knowledge, trash, played, board, valid_actions, hints):
        ...
        scores = list(map(lambda p: pretend_discard(p, knowledge[nr], board, trash), possible))
        def format_term(x):
            (col, rank, n, prob, val) = x
            return COLORNAMES[col] + " " + str(rank) + " (%.2f%%): %.2f"%(prob*100, val)

        self.explanation.append(["Discard Scores"] + list(map(lambda x: "\n".join(map(format_term, x[2])) + "\n%.2f"%(x[1]), scores)))
...
class Game(object):
    ...
    def perform(self, action):
        ...
        if action.type == HINT_COLOR:
            ...
            print(self.players[self.current_player].name, "plays", format_card((col,num)), file=self.log)
